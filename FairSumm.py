...
def extractDocumentCorpus():
	...
	fileptr.close()
	document_to_senctence_corpus['source'] = fileText
...
def extractSummary(cluster_to_sentences_dict):
	...
	for cluster in cluster_to_sentences_dict:
		...
		for each_sentence in current_cluster :
			if (each_sentence not in Summary) and (len(each_sentence.split())> inf):
...
def main():
	...
	document_to_senctence_corpus = extractDocumentCorpus()
